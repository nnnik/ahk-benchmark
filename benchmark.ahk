...
FormatResult(reps, startTick, endTick, ticksPerSec, name = "Benchmark") {
	...
	MsgBox,,%name%,Reps: %reps%`nStart Tick: %startTick%`nEnd Tick: %endTick%`nTick Frequency: %ticksPerSec%tps`nElapsed Time: %elpTime%%elpSfx%`nTime Per Rep: %repTime%%repSfx%`n`n-- Environment Info --`nAHK Version: %A_AhkVersion%`nOS Version: %A_OSVersion%`nIs 64bit OS: %A_Is64bitOS%`nError Level: %ErrorLevel%`nLast Error: %A_LastError%
}